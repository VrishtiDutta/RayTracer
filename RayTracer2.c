...
void buildScene(void)
{
 ...
 o=newPlane(0.05,.75,.05,.05,.55,.8,.75,1,1,2);	// Note the plane is highly-reflective (rs=rg=.75) so we
 ...
 o=newPlane(0.05,.75,.05,.05,.95,.95,.45,1,1,2);//this is the light source
 ...
 o=newSphere(0.05,.95,.35,.35,1,.25,.25,1,1,35);
 Scale(o,.75,.5,1.5);
 RotateY(o,PI/2);
 ...
 o=newSphere(0.05,.95,.95,.75,.75,.95,.55,1,1,35);
 Scale(o,.5,2.0,1.0);
 RotateZ(o,PI/1.5);
 Translate(o,1.75,1.25,5.0);
 o->isLightSource=0;
	 ...
	 // fprintf(stderr,"%d/%d, ",j,sx);
		...
		for(x=0; x<(sample_d-1)*antialiasing+1; x++){
			for(y=0; y<(sample_d-1)*antialiasing+1; y++){
				...
				subVectors(origin, d);
				normalize(d);
				//printf("subpixel d %f %f %f\n", dhttps://www.facebook.com/->px, d->py, d->pz);
				d->pw = 0;
				ray = newRay(origin, d);	
				ray->p0 = *origin;
				ray->d = *d;
