...
// Global Variables for scene creation
int MAX_DEPTH;
...
void rtShade(struct object3D *obj, struct point3D *p, struct point3D *n, struct ray3D *ray, int depth, double a, double b, struct colourRGB *col)
{
 ...
 //
	...
	// This will hold the colour as all the components of
	// the Phong illumination model are processed
	tmp_col.R=0;
	tmp_col.G=0;
	tmp_col.B=0;
	...
	else
	{
	    ...
	    // for the object. Note that textures are also used for Photon Mapping.
...
void findFirstHit(struct ray3D *ray, double *lambda, struct object3D *Os, struct object3D **obj, struct point3D *p, struct point3D *n, double *a, double *b)
{
 ...
 //   - The pointer to the object at the intersection (so evaluate the colour in the shading function)
 //   - The location of the intersection point (in p)
 //   - The normal at the intersection point (in n)
 //
 // Os is the 'source' object for the ray being processed, can be NULL, and is used to ensure it doesn't 
 // return a self-intersection due to numerical errors for recursive raytrace calls.
 //
 //case 1: camera ray
 //case 2: shadow ray: Os is given. obj, p, n, is not. Want to know is if ray from Os to light intersects with any other objs
 //case 3: object reflection ray
	...
	*lambda = 0;	
	...
	}else{ //this ray checks for shadow at 
		while(curr_obj != NULL){ //comment out loop, just get just plane to render
			if(curr_obj!=Os){ 
				curr_obj->intersect(curr_obj, ray, &lam, tp, tn, a, b);
				if(lam>0 && curr_obj->isLightSource!=1){
					*lambda = lam;
				}
			}else{
				*lambda = 0;
	 ...
	 else
	 {
		...
		if (lambda > 0){ //intersection == true, color it
			...
			while (curr_light!=NULL)
			{
				...
				memcpy(&dist_v, dir, sizeof(point3D));
...
int main(int argc, char *argv[])
{
	...
	//back == index 1
	environment_map[1] = readPPMimage("3.ppm");
	//up == index 2
	environment_map[2] = readPPMimage("4.ppm");
	//down == index 3
	environment_map[3] = readPPMimage("2.ppm");
	//back == index 4
	...
	// To define the gaze vector,  choose a point 'pc' in the scene that
	// the camera is looking at, and do the vector subtraction pc-e.
	// Here: set up the camera to be looking at the origin, so g=(0,0,0)-(0,0,-1)
	g.px=0;
	g.py=0;
	g.pz=1;
	g.pw=0;
	...
	dv=-cam->wsize/(sx-1);		// here use wl, wt, and wsize. du=dv since the image is
	...
	for (j=0;j<sx;j++)
	{
	 	for (i=0;i<sx;i++)
	 	{
			...
			//want to super-sample by spliting each pixel into 16 subpixels
			...
			//coloring the pixel with u and v values -- for debug only
